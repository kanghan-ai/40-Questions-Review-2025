...
const getApiKey = () => {
  return (import.meta as any).env.VITE_BIGMODEL_API_KEY ||
    (import.meta as any).env.VITE_OPENAI_API_KEY ||
    (import.meta as any).env.API_KEY;
};

const getBaseURL = () => {
  return (import.meta as any).env.VITE_BIGMODEL_BASE_URL ||
    (import.meta as any).env.VITE_OPENAI_BASE_URL ||
    "https://open.bigmodel.cn/api/paas/v4/";
};

const getModel = () => {
  return (import.meta as any).env.VITE_OPENAI_MODEL || "glm-4.7-flash";
};
...
const getClient = () => {
  ...
  if (!apiKey || apiKey.includes('你的_API_KEY_粘贴在这里')) {
    console.error("DEBUG: API Key detected as:", apiKey);
    throw new Error("Missing OpenAI API key. 请在 .env.local 中填写正确的 VITE_BIGMODEL_API_KEY 并重启服务。");
  }

  const baseURL = sanitizeBaseURL(getBaseURL());
