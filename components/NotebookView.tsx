...
import { X, ArrowLeft, ArrowRight, Download, Loader2 } from 'lucide-react';
...
export const NotebookView: React.FC<Props> = ({ answers, summary, onClose }) => {
  ...
  return (
    ...
    >
      ...
      {isDownloading && (
        <div className="fixed inset-0 z-[10001] bg-black/40 backdrop-blur-sm flex flex-col items-center justify-center fade-in">
          <div className="bg-stone-900/80 p-8 rounded-2xl border border-stone-700/50 flex flex-col items-center space-y-4 shadow-2xl">
            <Loader2 className="w-10 h-10 text-amber-200 animate-spin" />
            <div className="text-center">
              <p className="text-amber-50 text-lg font-serif font-bold tracking-widest">正在装订画册...</p>
              <p className="text-stone-400 font-typewriter text-[10px] uppercase tracking-[0.2em] mt-1">Arranging Digital Album</p>
            </div>
          </div>
        </div>
      )}
