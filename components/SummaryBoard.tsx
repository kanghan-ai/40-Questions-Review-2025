...
import { RefreshCw, Upload, BookOpen, Camera, Heart, Star, Music, RotateCw, X, Sun, Zap, Coffee, Smile, Anchor, Cloud, Loader2 } from 'lucide-react';
...
export const SummaryBoard: React.FC<Props> = ({ summary, allAnswers, onRetake }) => {
  ...
  const spawnCard = (index: number) => {
    ...
    // 1. Horizontal Linear Distribution
    const margin = 80;
    const usableW = boardW - margin * 2;
    const cellW = usableW / total;

    // Calculate Slot Center
    const centerX = margin + cellW * (index + 0.5);

    // 2. Vertical Staggering (matching the reference image's feel)
    // index 0 (Note): slightly lower
    // index 1 (Ticket): higher 
    // index 2 (Paper): centered/large
    // index 3 (Polaroid): slightly higher
    const vOffsets = [40, -60, -20, -50];
    const centerY = (boardH / 2) + (vOffsets[index % vOffsets.length] || 0);

    // 3. Add light jitter for natural feel
    const jitterX = (Math.random() - 0.5) * 40;
    const jitterY = (Math.random() - 0.5) * 30;

    const targetX = centerX - 150 + jitterX;
    const targetY = centerY - 200 + jitterY;

    // 4. Subtle rotation
    const rotations = [-3, 2, -1, 4]; // More deliberate rotations
    const rotation = rotations[index % rotations.length] + (Math.random() * 4 - 2);
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (dragItemRef.current) {
      ...
      // Update position using left/top instead of transform to help html2canvas
  ...
  const handleScreenshot = async () => {
    ...
    if (boardRef.current && !isCapturing) {
      ...
      try {
        ...
        // 2. Small buffer for texture rendering
        await new Promise(r => setTimeout(r, 500));

        // 3. Capture with high fidelity using html-to-image
        const dataUrl = await toPng(boardRef.current, {
          quality: 1.0,
          pixelRatio: 3, // High def
          skipFonts: false,
          style: {
            // Force some stability
            transform: 'scale(1)',
          },
          filter: (node) => {
            const classList = (node as HTMLElement).classList;
            return classList ? !classList.contains('no-screenshot') : true;
          }
        });
  ...
  return (
    <>
      ...
      >
        ...
        {isCapturing && (
          <div className="fixed inset-0 z-[10000] bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center no-screenshot fade-in">
            <div className="bg-white/10 p-8 rounded-2xl border border-white/20 flex flex-col items-center space-y-4">
              <Loader2 className="w-10 h-10 text-white animate-spin" />
              <div className="text-center">
                <p className="text-white font-serif text-lg font-bold tracking-widest">正在生成精美画卷...</p>
                <p className="text-white/60 font-typewriter text-[10px] uppercase tracking-[0.2em] mt-1">Developing High-Res Image</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="absolute inset-0 w-full h-full overflow-hidden">
          {items.map((item, i) => (
            <div
              ...
              className={`absolute group no-select ${item.id.startsWith('card') && parseInt(item.id.split('-')[1]) === items.length - 1 ? 'fly-in' : ''}`}
              style={{
                left: `${item.x}px`,
                top: `${item.y}px`,
        ...
        <div className="absolute bottom-8 right-8 flex flex-col gap-4 z-[9999] no-screenshot">
          <button
            ...
            className="relative bg-stone-800 text-white w-14 h-14 rounded-full shadow-2xl hover:bg-stone-700 hover:scale-110 transition-all flex items-center justify-center border-2 border-stone-600 group disabled:opacity-50"
          >
            <Camera className={`w-6 h-6 ${isCapturing ? 'animate-pulse' : ''}`} />
            <span className="absolute right-full mr-4 px-3 py-1.5 bg-stone-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all pointer-events-none whitespace-nowrap shadow-xl font-serif">
              保存图片
            </span>
          </button>

          <label className="relative cursor-pointer bg-white text-stone-800 w-14 h-14 rounded-full shadow-2xl hover:scale-110 transition-all flex items-center justify-center border-2 border-stone-100 group">
          ...
          <button
            ...
            className="relative bg-amber-100 text-amber-900 w-14 h-14 rounded-full shadow-2xl hover:bg-amber-200 hover:scale-110 transition-all flex items-center justify-center border-2 border-amber-300 group"
          >
          ...
          <button
            onClick={onRetake}
            className="relative bg-stone-800 text-stone-100 w-14 h-14 rounded-full shadow-2xl hover:bg-stone-700 hover:scale-110 transition-all flex items-center justify-center border-2 border-stone-700 group"
          >
